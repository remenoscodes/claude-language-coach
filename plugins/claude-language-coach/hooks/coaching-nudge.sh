...
cat << 'EOF'
{
  "hookSpecificOutput": {
    ...
    "additionalContext": "Language coaching is active. After completing the user's task, check their message for language patterns (grammar, spelling, false friends, interference) in their configured target languages. If corrections or teaching opportunities found, append a coaching block at the end of your response. Follow the coaching instructions in the user's CLAUDE.md under '# Language Coaching Instructions'. Memory files at ~/.claude/coaching/{lang}-coaching.json."
  }
}
EOF
exit 0
