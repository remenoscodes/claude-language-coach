...
### Added
- **Progress skill**: `/progress` (or `/claude-language-coach:progress`) shows longitudinal learning metrics — corrections by type, pattern lifecycle, vocabulary growth, session timeline, SRS health, and actionable insights. Accepts optional language argument (`/progress en`, `/progress all`). Defaults to `all`. Read-only, no modifications to coaching files.
...
### Fixed
- **Flatten plugin structure**: moved `.claude-plugin/`, `hooks/`, and `skills/` from `plugins/claude-language-coach/` to the repo root. Claude Code resolves hook paths relative to the repo root, so the nested structure prevented the UserPromptSubmit hook from firing.
